...
```js
   ...
   window.onload = function() {
     ...
     const campusesContainer = document.getElementById("gaCampuses");
     // The getElementsByTagName() method returns a live HTMLCollection of elements with the given tag name.
     const gaCampuses = campusesContainer.getElementsByTagName("li");
...
The first thing we need to do if we're going to use jQuery is to include the source code in our html file. Put this script tag right above your custom script import (which is typically right above the closing body tag!).
