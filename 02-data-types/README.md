...
#### Part 1: Always Use let / const!

We declare new variables in JavaScript using the let or const keywords.
...
First array (using `new` keyword)
  ...
  ```javascript
  const goods = new Array("water", "coffee")
  ```
...
const bigNums = evens.filter(function (num) {
  return num > 5
})
=> undefined
...
const smallNums = odds.filter(function (num) {
  return num < 5
})
=> undefined
smallNums
...
```javascript
const timesFive = evens.map(function (num) {
  return num * 5
})
=> undefined
...
const timesTen = odds.map(function (num) {
  return num * 10
})
=> undefined
